...
final class S3FlexibleChecksumsTests: S3XCTestCase {
    ...
    private func testPutGetObject(
        withChecksumAlgorithm algorithm: S3ClientTypes.ChecksumAlgorithm,
        objectNameSuffix: String, upload: ByteStream
        ) async throws {
        ...
        let getInput = GetObjectInput(
            bucket: bucketName,
            checksumMode: S3ClientTypes.ChecksumMode.enabled,
            key: objectName
            )
        let responseGet = try await client.getObject(
            input: getInput
            ) // will error for normal payloads if checksum mismatch
        XCTAssertNotNil(responseGet.body) // Ensure there's a body in the response.
    }

    private func getChecksumResponse(
        from response: PutObjectOutput,
        with algorithm: S3ClientTypes.ChecksumAlgorithm
        ) throws -> String? {
        switch algorithm {
        case .crc32:
    ...
    func test_putGetObject_data_crc32() async throws {
        try await testPutGetObject(
            withChecksumAlgorithm: .crc32,
            objectNameSuffix: "crc32-data",
            upload: .data(Data("Hello, world!".utf8))
        )
    }
