...
public protocol MarketplaceCommerceAnalyticsClientProtocol {
    ...
    func generateDataSet(input: GenerateDataSetInput, completion: @escaping (SdkResult<GenerateDataSetOutput, GenerateDataSetOutputError>) -> Void)
    ...
    ///         Requires a Role with an attached permissions policy providing Allow permissions for the following actions:
    ///         s3:PutObject, s3:GetBucketLocation, sns:GetTopicAttributes, sns:Publish, iam:GetRolePolicy.
    func startSupportDataExport(input: StartSupportDataExportInput, completion: @escaping (SdkResult<StartSupportDataExportOutput, StartSupportDataExportOutputError>) -> Void)
}
