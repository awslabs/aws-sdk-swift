        ...
        if let exportTaskIds = exportTaskIds {
            if !exportTaskIds.isEmpty {
                for (index0, exporttaskid0) in exportTaskIds.enumerated() {
                    var exportTaskIdsContainer0 = container.nestedContainer(keyedBy: Key.self, forKey: Key("ExportTaskId.\(index0.advanced(by: 1))"))
                    try exportTaskIdsContainer0.encode(exporttaskid0, forKey: Key(""))
                }
            }
        }
