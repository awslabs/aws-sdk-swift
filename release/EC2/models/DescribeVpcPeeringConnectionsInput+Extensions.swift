        ...
        if let vpcPeeringConnectionIds = vpcPeeringConnectionIds {
            if !vpcPeeringConnectionIds.isEmpty {
                for (index0, vpcpeeringconnectionid0) in vpcPeeringConnectionIds.enumerated() {
                    var vpcPeeringConnectionIdsContainer0 = container.nestedContainer(keyedBy: Key.self, forKey: Key("VpcPeeringConnectionId.\(index0.advanced(by: 1))"))
                    try vpcPeeringConnectionIdsContainer0.encode(vpcpeeringconnectionid0, forKey: Key(""))
                }
            }
        }
        try container.encode("DescribeVpcPeeringConnections", forKey:Key("Action"))
