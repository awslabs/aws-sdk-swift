...
extension CreateVolumePermissionModifications: Codable, Reflection {
        ...
        if let add = add {
            if !add.isEmpty {
                for (index0, createvolumepermission0) in add.enumerated() {
                    ...
                    try addContainer0.encode(createvolumepermission0, forKey: Key(""))
                }
            }
        }
        if let remove = remove {
            if !remove.isEmpty {
                for (index0, createvolumepermission0) in remove.enumerated() {
                    ...
                    try removeContainer0.encode(createvolumepermission0, forKey: Key(""))
                }
            }
        }
    }
    ...
    public init (from decoder: Decoder) throws {
        ...
        if containerValues.contains(.add) {
            ...
            if let addWrappedContainer = addWrappedContainer {
                let addContainer = try addWrappedContainer.decodeIfPresent([CreateVolumePermission].self, forKey: .member)
                var addBuffer:[CreateVolumePermission]? = nil
                if let addContainer = addContainer {
                    addBuffer = [CreateVolumePermission]()
        ...
        if containerValues.contains(.remove) {
            ...
            if let removeWrappedContainer = removeWrappedContainer {
                let removeContainer = try removeWrappedContainer.decodeIfPresent([CreateVolumePermission].self, forKey: .member)
                var removeBuffer:[CreateVolumePermission]? = nil
                if let removeContainer = removeContainer {
                    removeBuffer = [CreateVolumePermission]()
