...
extension EnableFastSnapshotRestoreErrorItem: Codable, Reflection {
        ...
        if let fastSnapshotRestoreStateErrors = fastSnapshotRestoreStateErrors {
            if !fastSnapshotRestoreStateErrors.isEmpty {
                for (index0, enablefastsnapshotrestorestateerroritem0) in fastSnapshotRestoreStateErrors.enumerated() {
                    ...
                    try fastSnapshotRestoreStateErrorsContainer0.encode(enablefastsnapshotrestorestateerroritem0, forKey: Key(""))
                }
            }
        }
    ...
    public init (from decoder: Decoder) throws {
        ...
        if containerValues.contains(.fastSnapshotRestoreStateErrors) {
            ...
            if let fastSnapshotRestoreStateErrorsWrappedContainer = fastSnapshotRestoreStateErrorsWrappedContainer {
                let fastSnapshotRestoreStateErrorsContainer = try fastSnapshotRestoreStateErrorsWrappedContainer.decodeIfPresent([EnableFastSnapshotRestoreStateErrorItem].self, forKey: .member)
                var fastSnapshotRestoreStateErrorsBuffer:[EnableFastSnapshotRestoreStateErrorItem]? = nil
                if let fastSnapshotRestoreStateErrorsContainer = fastSnapshotRestoreStateErrorsContainer {
                    fastSnapshotRestoreStateErrorsBuffer = [EnableFastSnapshotRestoreStateErrorItem]()
