...
extension NewDhcpConfiguration: Codable, Reflection {
        ...
        if let values = values {
            if !values.isEmpty {
                for (index0, string0) in values.enumerated() {
                    var valuesContainer0 = container.nestedContainer(keyedBy: Key.self, forKey: Key("Value.\(index0.advanced(by: 1))"))
