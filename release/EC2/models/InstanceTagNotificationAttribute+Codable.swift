...
extension InstanceTagNotificationAttribute: Codable, Reflection {
        ...
        if let instanceTagKeys = instanceTagKeys {
            if !instanceTagKeys.isEmpty {
                for (index0, string0) in instanceTagKeys.enumerated() {
                    var instanceTagKeysContainer0 = container.nestedContainer(keyedBy: Key.self, forKey: Key("InstanceTagKeySet.\(index0.advanced(by: 1))"))
