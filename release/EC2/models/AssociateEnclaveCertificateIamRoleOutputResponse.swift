// Code generated by smithy-swift-codegen. DO NOT EDIT!



public struct AssociateEnclaveCertificateIamRoleOutputResponse: Equatable {
    /// <p>The name of the Amazon S3 bucket to which the certificate was uploaded.</p>
    public let certificateS3BucketName: String?
    /// <p>The Amazon S3 object key where the certificate, certificate chain, and encrypted private key bundle are stored. The
    /// 			object key is formatted as follows:  <code>role_arn</code>/<code>certificate_arn</code>.</p>
    public let certificateS3ObjectKey: String?
    /// <p>The ID of the AWS KMS CMK used to encrypt the private key of the certificate.</p>
    public let encryptionKmsKeyId: String?

    public init (
        certificateS3BucketName: String? = nil,
        certificateS3ObjectKey: String? = nil,
        encryptionKmsKeyId: String? = nil
    )
    {
        self.certificateS3BucketName = certificateS3BucketName
        self.certificateS3ObjectKey = certificateS3ObjectKey
        self.encryptionKmsKeyId = encryptionKmsKeyId
    }
}

extension AssociateEnclaveCertificateIamRoleOutputResponse: CustomDebugStringConvertible {
    public var debugDescription: String {
        "AssociateEnclaveCertificateIamRoleOutputResponse(certificateS3BucketName: \(String(describing: certificateS3BucketName)), certificateS3ObjectKey: \(String(describing: certificateS3ObjectKey)), encryptionKmsKeyId: \(String(describing: encryptionKmsKeyId)))"}
}
