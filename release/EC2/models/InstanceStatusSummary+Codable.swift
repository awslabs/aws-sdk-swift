...
extension InstanceStatusSummary: Codable, Reflection {
        ...
        if let details = details {
            if !details.isEmpty {
                for (index0, instancestatusdetails0) in details.enumerated() {
                    ...
                    try detailsContainer0.encode(instancestatusdetails0, forKey: Key(""))
                }
            }
        }
    ...
    public init (from decoder: Decoder) throws {
        ...
        if containerValues.contains(.details) {
            ...
            if let detailsWrappedContainer = detailsWrappedContainer {
                let detailsContainer = try detailsWrappedContainer.decodeIfPresent([InstanceStatusDetails].self, forKey: .member)
                var detailsBuffer:[InstanceStatusDetails]? = nil
                if let detailsContainer = detailsContainer {
                    detailsBuffer = [InstanceStatusDetails]()
        ...
        let statusDecoded = try containerValues.decodeIfPresent(SummaryStatus.self, forKey: .status)
