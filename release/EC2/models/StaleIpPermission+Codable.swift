...
extension StaleIpPermission: Codable, Reflection {
        ...
        if let ipRanges = ipRanges {
            if !ipRanges.isEmpty {
                for (index0, string0) in ipRanges.enumerated() {
                    ...
                    try ipRangesContainer0.encode(string0, forKey: Key(""))
                }
            }
        }
    ...
    public init (from decoder: Decoder) throws {
        ...
        if containerValues.contains(.ipRanges) {
            ...
            if let ipRangesWrappedContainer = ipRangesWrappedContainer {
                let ipRangesContainer = try ipRangesWrappedContainer.decodeIfPresent([String].self, forKey: .member)
                var ipRangesBuffer:[String]? = nil
                if let ipRangesContainer = ipRangesContainer {
                    ipRangesBuffer = [String]()
        ...
        if containerValues.contains(.prefixListIds) {
            ...
            if let prefixListIdsWrappedContainer = prefixListIdsWrappedContainer {
                let prefixListIdsContainer = try prefixListIdsWrappedContainer.decodeIfPresent([String].self, forKey: .member)
                var prefixListIdsBuffer:[String]? = nil
                if let prefixListIdsContainer = prefixListIdsContainer {
                    prefixListIdsBuffer = [String]()
