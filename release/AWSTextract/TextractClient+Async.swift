...
public extension TextractClient {
    ...
    {
        typealias analyzeIDContinuation = CheckedContinuation<AnalyzeIDOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: analyzeIDContinuation) in
            analyzeID(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias getExpenseAnalysisContinuation = CheckedContinuation<GetExpenseAnalysisOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: getExpenseAnalysisContinuation) in
            getExpenseAnalysis(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias startExpenseAnalysisContinuation = CheckedContinuation<StartExpenseAnalysisOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: startExpenseAnalysisContinuation) in
            startExpenseAnalysis(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
