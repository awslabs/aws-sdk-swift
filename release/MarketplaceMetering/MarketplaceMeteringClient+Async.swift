...
public extension MarketplaceMeteringClient {
    ...
    func batchMeterUsage(input: BatchMeterUsageInput) async throws -> BatchMeterUsageOutputResponse
    {
        typealias batchMeterUsageContinuation = CheckedContinuation<BatchMeterUsageOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: batchMeterUsageContinuation) in
            batchMeterUsage(input: input) { result in
                switch result {
    ...
    ///             define).</p>
    func meterUsage(input: MeterUsageInput) async throws -> MeterUsageOutputResponse
    {
        typealias meterUsageContinuation = CheckedContinuation<MeterUsageOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: meterUsageContinuation) in
            meterUsage(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    ///             </li>
    ///          </ul>
    func registerUsage(input: RegisterUsageInput) async throws -> RegisterUsageOutputResponse
    {
        typealias registerUsageContinuation = CheckedContinuation<RegisterUsageOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: registerUsageContinuation) in
            registerUsage(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    func resolveCustomer(input: ResolveCustomerInput) async throws -> ResolveCustomerOutputResponse
    {
        typealias resolveCustomerContinuation = CheckedContinuation<ResolveCustomerOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: resolveCustomerContinuation) in
            resolveCustomer(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
