...
public extension NetworkFirewallClient {
    ...
    {
        typealias describeRuleGroupMetadataContinuation = CheckedContinuation<DescribeRuleGroupMetadataOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: describeRuleGroupMetadataContinuation) in
            describeRuleGroupMetadata(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
