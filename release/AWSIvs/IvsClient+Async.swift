...
public extension IvsClient {
    ...
    {
        typealias getStreamSessionContinuation = CheckedContinuation<GetStreamSessionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: getStreamSessionContinuation) in
            getStreamSession(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias listStreamSessionsContinuation = CheckedContinuation<ListStreamSessionsOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: listStreamSessionsContinuation) in
            listStreamSessions(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
