// Code generated by smithy-swift-codegen. DO NOT EDIT!

import ClientRuntime

public struct DescribeEdgePackagingJobOutputResponse: Equatable {
    /// <p>The name of the SageMaker Neo compilation job that is used to locate model artifacts that are being packaged.</p>
    public let compilationJobName: String?
    /// <p>The timestamp of when the packaging job was created.</p>
    public let creationTime: Date?
    /// <p>The Amazon Resource Name (ARN) of the edge packaging job.</p>
    public let edgePackagingJobArn: String?
    /// <p>The name of the edge packaging job.</p>
    public let edgePackagingJobName: String?
    /// <p>The current status of the packaging job.</p>
    public let edgePackagingJobStatus: EdgePackagingJobStatus?
    /// <p>Returns a message describing the job status and error messages.</p>
    public let edgePackagingJobStatusMessage: String?
    /// <p>The timestamp of when the job was last updated.</p>
    public let lastModifiedTime: Date?
    /// <p>The Amazon Simple Storage (S3) URI where model artifacts ares stored.</p>
    public let modelArtifact: String?
    /// <p>The name of the model.</p>
    public let modelName: String?
    /// <p>The signature document of files in the model artifact.</p>
    public let modelSignature: String?
    /// <p>The version of the model.</p>
    public let modelVersion: String?
    /// <p>The output configuration for the edge packaging job.</p>
    public let outputConfig: EdgeOutputConfig?
    /// <p>The CMK to use when encrypting the EBS volume the job run on.</p>
    public let resourceKey: String?
    /// <p>The Amazon Resource Name (ARN) of an IAM role that enables Amazon SageMaker to  download and upload the model, and to contact Neo.</p>
    public let roleArn: String?

    public init (
        compilationJobName: String? = nil,
        creationTime: Date? = nil,
        edgePackagingJobArn: String? = nil,
        edgePackagingJobName: String? = nil,
        edgePackagingJobStatus: EdgePackagingJobStatus? = nil,
        edgePackagingJobStatusMessage: String? = nil,
        lastModifiedTime: Date? = nil,
        modelArtifact: String? = nil,
        modelName: String? = nil,
        modelSignature: String? = nil,
        modelVersion: String? = nil,
        outputConfig: EdgeOutputConfig? = nil,
        resourceKey: String? = nil,
        roleArn: String? = nil
    )
    {
        self.compilationJobName = compilationJobName
        self.creationTime = creationTime
        self.edgePackagingJobArn = edgePackagingJobArn
        self.edgePackagingJobName = edgePackagingJobName
        self.edgePackagingJobStatus = edgePackagingJobStatus
        self.edgePackagingJobStatusMessage = edgePackagingJobStatusMessage
        self.lastModifiedTime = lastModifiedTime
        self.modelArtifact = modelArtifact
        self.modelName = modelName
        self.modelSignature = modelSignature
        self.modelVersion = modelVersion
        self.outputConfig = outputConfig
        self.resourceKey = resourceKey
        self.roleArn = roleArn
    }
}

extension DescribeEdgePackagingJobOutputResponse: CustomDebugStringConvertible {
    public var debugDescription: String {
        "DescribeEdgePackagingJobOutputResponse(compilationJobName: \(String(describing: compilationJobName)), creationTime: \(String(describing: creationTime)), edgePackagingJobArn: \(String(describing: edgePackagingJobArn)), edgePackagingJobName: \(String(describing: edgePackagingJobName)), edgePackagingJobStatus: \(String(describing: edgePackagingJobStatus)), edgePackagingJobStatusMessage: \(String(describing: edgePackagingJobStatusMessage)), lastModifiedTime: \(String(describing: lastModifiedTime)), modelArtifact: \(String(describing: modelArtifact)), modelName: \(String(describing: modelName)), modelSignature: \(String(describing: modelSignature)), modelVersion: \(String(describing: modelVersion)), outputConfig: \(String(describing: outputConfig)), resourceKey: \(String(describing: resourceKey)), roleArn: \(String(describing: roleArn)))"}
}
