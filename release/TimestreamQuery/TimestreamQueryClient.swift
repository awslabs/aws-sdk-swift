...
extension TimestreamQueryClient: TimestreamQueryClientProtocol {
    ...
    ///     </p>
    public func cancelQuery(input: CancelQueryInput, completion: @escaping (SdkResult<CancelQueryOutput, CancelQueryOutputError>) -> Void)
    {
        let urlPath = "/"
        ...
        var operation = OperationStack<CancelQueryInput, CancelQueryOutput, CancelQueryOutputError>(id: "cancelQuery")
        ...
        operation.serializeStep.intercept(position: .before, middleware: ContentTypeMiddleware<CancelQueryInput, CancelQueryOutput, CancelQueryOutputError>(contentType: "application/x-amz-json-1.0"))
        ...
        operation.serializeStep.intercept(position: .before, middleware: XAmzTargetMiddleware<CancelQueryInput, CancelQueryOutput, CancelQueryOutputError>(xAmzTarget: "Timestream_20181101.CancelQuery"))
    ...
    public func query(input: QueryInput, completion: @escaping (SdkResult<QueryOutput, QueryOutputError>) -> Void)
    {
        ...
        operation.addDefaultOperationMiddlewares()
        operation.initializeStep.intercept(position: .before, id: "IdempotencyTokenMiddleware") { (context, input, next) -> Result<OperationOutput<QueryOutput>, SdkError<QueryOutputError>> in
        ...
        operation.finalizeStep.intercept(position: .before,
                                                 middleware: SigV4Middleware(config: sigv4Config))
        operation.buildStep.intercept(position: .before, middleware: UserAgentMiddleware(metadata: AWSUserAgentMetadata.fromEnv(apiMetadata: APIMetadata(serviceId: serviceName, version: "1.0"))))
        operation.serializeStep.intercept(position: .before, middleware: XAmzTargetMiddleware<QueryInput, QueryOutput, QueryOutputError>(xAmzTarget: "Timestream_20181101.Query"))
        let result = operation.handleMiddleware(context: context.build(), input: input, next: client.getHandler())
        completion(result)
    }
