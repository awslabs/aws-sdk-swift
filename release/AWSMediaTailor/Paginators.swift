...
extension PaginatorSequence where Input == GetChannelScheduleInput, Output == GetChannelScheduleOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.ScheduleEntry] {
...
extension PaginatorSequence where Input == ListChannelsInput, Output == ListChannelsOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.Channel] {
        return try await self.asyncCompactMap { item in item.items }
    }
}

/// Paginate over `[ListLiveSourcesOutputResponse]` results.
///
/// When this operation is called, an `AsyncSequence` is created. AsyncSequences are lazy so no service
/// calls are made until the sequence is iterated over. This also means there is no guarantee that the request is valid
/// until then. If there are errors in your request, you will see the failures only after you start iterating.
/// - Parameters:
///     - input: A `[ListLiveSourcesInput]` to start pagination
/// - Returns: An `AsyncSequence` that can iterate over `ListLiveSourcesOutputResponse`
extension MediaTailorClient {
    public func listLiveSourcesPaginated(input: ListLiveSourcesInput) -> ClientRuntime.PaginatorSequence<ListLiveSourcesInput, ListLiveSourcesOutputResponse> {
        return ClientRuntime.PaginatorSequence<ListLiveSourcesInput, ListLiveSourcesOutputResponse>(input: input, inputKey: \ListLiveSourcesInput.nextToken, outputKey: \ListLiveSourcesOutputResponse.nextToken, paginationFunction: self.listLiveSources(input:))
    }
}

extension ListLiveSourcesInput: ClientRuntime.PaginateToken {
    public func usingPaginationToken(_ token: Swift.String) -> ListLiveSourcesInput {
        return ListLiveSourcesInput(
            maxResults: self.maxResults,
            nextToken: token,
...
/// This paginator transforms the `AsyncSequence` returned by `listLiveSourcesPaginated`
/// to access the nested member `[MediaTailorClientTypes.LiveSource]`
/// - Returns: `[MediaTailorClientTypes.LiveSource]`
extension PaginatorSequence where Input == ListLiveSourcesInput, Output == ListLiveSourcesOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.LiveSource] {
        return try await self.asyncCompactMap { item in item.items }
    }
}
...
extension PaginatorSequence where Input == ListPlaybackConfigurationsInput, Output == ListPlaybackConfigurationsOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.PlaybackConfiguration] {
        return try await self.asyncCompactMap { item in item.items }
    }
}
...
extension PaginatorSequence where Input == ListPrefetchSchedulesInput, Output == ListPrefetchSchedulesOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.PrefetchSchedule] {
        return try await self.asyncCompactMap { item in item.items }
    }
}
...
extension PaginatorSequence where Input == ListSourceLocationsInput, Output == ListSourceLocationsOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.SourceLocation] {
        return try await self.asyncCompactMap { item in item.items }
    }
}
...
extension PaginatorSequence where Input == ListVodSourcesInput, Output == ListVodSourcesOutputResponse {
    public func items() async throws -> [MediaTailorClientTypes.VodSource] {
        return try await self.asyncCompactMap { item in item.items }
    }
}
