...
public extension Route53DomainsClient {
    ...
    {
        typealias listPricesContinuation = CheckedContinuation<ListPricesOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: listPricesContinuation) in
            listPrices(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
