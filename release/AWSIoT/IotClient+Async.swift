...
public extension IotClient {
    ...
    {
        typealias describeManagedJobTemplateContinuation = CheckedContinuation<DescribeManagedJobTemplateOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: describeManagedJobTemplateContinuation) in
            describeManagedJobTemplate(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias listManagedJobTemplatesContinuation = CheckedContinuation<ListManagedJobTemplatesOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: listManagedJobTemplatesContinuation) in
            listManagedJobTemplates(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
