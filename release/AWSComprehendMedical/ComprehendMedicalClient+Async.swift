...
public extension ComprehendMedicalClient {
    ...
    {
        typealias describeSNOMEDCTInferenceJobContinuation = CheckedContinuation<DescribeSNOMEDCTInferenceJobOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: describeSNOMEDCTInferenceJobContinuation) in
            describeSNOMEDCTInferenceJob(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias inferSNOMEDCTContinuation = CheckedContinuation<InferSNOMEDCTOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: inferSNOMEDCTContinuation) in
            inferSNOMEDCT(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias listSNOMEDCTInferenceJobsContinuation = CheckedContinuation<ListSNOMEDCTInferenceJobsOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: listSNOMEDCTInferenceJobsContinuation) in
            listSNOMEDCTInferenceJobs(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias startSNOMEDCTInferenceJobContinuation = CheckedContinuation<StartSNOMEDCTInferenceJobOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: startSNOMEDCTInferenceJobContinuation) in
            startSNOMEDCTInferenceJob(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias stopSNOMEDCTInferenceJobContinuation = CheckedContinuation<StopSNOMEDCTInferenceJobOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: stopSNOMEDCTInferenceJobContinuation) in
            stopSNOMEDCTInferenceJob(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
