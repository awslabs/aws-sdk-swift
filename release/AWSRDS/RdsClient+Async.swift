...
public extension RdsClient {
    ...
    {
        typealias createCustomDBEngineVersionContinuation = CheckedContinuation<CreateCustomDBEngineVersionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: createCustomDBEngineVersionContinuation) in
            createCustomDBEngineVersion(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias deleteCustomDBEngineVersionContinuation = CheckedContinuation<DeleteCustomDBEngineVersionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: deleteCustomDBEngineVersionContinuation) in
            deleteCustomDBEngineVersion(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias modifyCustomDBEngineVersionContinuation = CheckedContinuation<ModifyCustomDBEngineVersionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: modifyCustomDBEngineVersionContinuation) in
            modifyCustomDBEngineVersion(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias rebootDBClusterContinuation = CheckedContinuation<RebootDBClusterOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: rebootDBClusterContinuation) in
            rebootDBCluster(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
