// Code generated by smithy-swift-codegen. DO NOT EDIT!



public struct BatchSuspendUserOutputResponse: Equatable {
    /// <p>If the <a>BatchSuspendUser</a> action fails for one or more of the user IDs in the
    ///             request, a list of the user IDs is returned, along with error codes and error messages.</p>
    public let userErrors: [UserError]?

    public init (
        userErrors: [UserError]? = nil
    )
    {
        self.userErrors = userErrors
    }
}

extension BatchSuspendUserOutputResponse: CustomDebugStringConvertible {
    public var debugDescription: String {
        "BatchSuspendUserOutputResponse(userErrors: \(String(describing: userErrors)))"}
}
