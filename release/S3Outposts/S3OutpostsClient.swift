...
extension S3OutpostsClient: S3OutpostsClientProtocol {
    ...
    public func createEndpoint(input: CreateEndpointInput, completion: @escaping (SdkResult<CreateEndpointOutput, CreateEndpointOutputError>) -> Void)
    {
        ...
        operation.serializeStep.intercept(position: .before, middleware: ContentTypeMiddleware<CreateEndpointInput, CreateEndpointOutput, CreateEndpointOutputError>(contentType: "application/json"))
    ...
    public func listEndpoints(input: ListEndpointsInput, completion: @escaping (SdkResult<ListEndpointsOutput, ListEndpointsOutputError>) -> Void)
    {
        ...
        operation.serializeStep.intercept(position: .before, middleware: ContentTypeMiddleware<ListEndpointsInput, ListEndpointsOutput, ListEndpointsOutputError>(contentType: "application/json"))
        operation.deserializeStep.intercept(position: .before, middleware: LoggerMiddleware(clientLogMode: config.clientLogMode))
        operation.buildStep.intercept(position: .before, middleware: EndpointResolverMiddleware(endpointResolver: config.endpointResolver, serviceId: serviceName))
        operation.finalizeStep.intercept(position: .after, middleware: RetrierMiddleware(retrier: config.retrier))
        let sigv4Config = SigV4Config(unsignedBody: false)
