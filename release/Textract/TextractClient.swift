...
extension TextractClient: TextractClientProtocol {
    ...
    public func analyzeExpense(input: AnalyzeExpenseInput, completion: @escaping (ClientRuntime.SdkResult<AnalyzeExpenseOutputResponse, AnalyzeExpenseOutputError>) -> Void)
    {
        ...
        let context = ClientRuntime.HttpContextBuilder()
                      ...
                      .withSigningName(value: "textract")
                      .withSigningRegion(value: config.signingRegion)
        var operation = OperationStack<AnalyzeExpenseInput, AnalyzeExpenseOutputResponse, AnalyzeExpenseOutputError>(id: "analyzeExpense")
        operation.addDefaultOperationMiddlewares()
        operation.serializeStep.intercept(position: .before, middleware: AnalyzeExpenseInputHeadersMiddleware())
        operation.serializeStep.intercept(position: .before, middleware: AnalyzeExpenseInputQueryItemMiddleware())
        ...
        operation.serializeStep.intercept(position: .before, middleware: AnalyzeExpenseInputBodyMiddleware())
        operation.deserializeStep.intercept(position: .before, middleware: ClientRuntime.LoggerMiddleware(clientLogMode: config.clientLogMode))
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.EndpointResolverMiddleware(endpointResolver: config.endpointResolver, serviceId: serviceName))
        operation.finalizeStep.intercept(position: .after, middleware: AWSClientRuntime.RetryerMiddleware(retryer: config.retryer))
        let sigv4Config = AWSClientRuntime.SigV4Config(unsignedBody: false)
