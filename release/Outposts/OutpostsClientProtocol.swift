// Code generated by smithy-swift-codegen. DO NOT EDIT!

import ClientRuntime

/// <p>AWS Outposts is a fully managed service that extends AWS infrastructure, APIs, and tools
///       to customer premises. By providing local access to AWS managed infrastructure, AWS Outposts
///       enables customers to build and run applications on premises using the same programming
///       interfaces as in AWS Regions, while using local compute and storage resources for lower
///       latency and local data processing needs.</p>
public protocol OutpostsClientProtocol {
    /// <p>Creates an Outpost.</p>
    func createOutpost(input: CreateOutpostInput, completion: @escaping (SdkResult<CreateOutpostOutput, CreateOutpostOutputError>) -> Void)
    /// <p>Deletes the Outpost.</p>
    func deleteOutpost(input: DeleteOutpostInput, completion: @escaping (SdkResult<DeleteOutpostOutput, DeleteOutpostOutputError>) -> Void)
    /// <p>Deletes the site.</p>
    func deleteSite(input: DeleteSiteInput, completion: @escaping (SdkResult<DeleteSiteOutput, DeleteSiteOutputError>) -> Void)
    /// <p>Gets information about the specified Outpost.</p>
    func getOutpost(input: GetOutpostInput, completion: @escaping (SdkResult<GetOutpostOutput, GetOutpostOutputError>) -> Void)
    /// <p>Lists the instance types for the specified Outpost.</p>
    func getOutpostInstanceTypes(input: GetOutpostInstanceTypesInput, completion: @escaping (SdkResult<GetOutpostInstanceTypesOutput, GetOutpostInstanceTypesOutputError>) -> Void)
    /// <p>List the Outposts for your AWS account.</p>
    func listOutposts(input: ListOutpostsInput, completion: @escaping (SdkResult<ListOutpostsOutput, ListOutpostsOutputError>) -> Void)
    /// <p>Lists the sites for the specified AWS account.</p>
    func listSites(input: ListSitesInput, completion: @escaping (SdkResult<ListSitesOutput, ListSitesOutputError>) -> Void)
    /// <p>Lists the tags for the specified resource.</p>
    func listTagsForResource(input: ListTagsForResourceInput, completion: @escaping (SdkResult<ListTagsForResourceOutput, ListTagsForResourceOutputError>) -> Void)
    /// <p>Adds tags to the specified resource.</p>
    func tagResource(input: TagResourceInput, completion: @escaping (SdkResult<TagResourceOutput, TagResourceOutputError>) -> Void)
    /// <p>Removes tags from the specified resource.</p>
    func untagResource(input: UntagResourceInput, completion: @escaping (SdkResult<UntagResourceOutput, UntagResourceOutputError>) -> Void)
}
