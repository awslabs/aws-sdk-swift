...
public extension KinesisClient {
    ...
    {
        typealias updateStreamModeContinuation = CheckedContinuation<UpdateStreamModeOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: updateStreamModeContinuation) in
            updateStreamMode(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
