...
public extension LocationClient {
    ...
    {
        typealias searchPlaceIndexForSuggestionsContinuation = CheckedContinuation<SearchPlaceIndexForSuggestionsOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: searchPlaceIndexForSuggestionsContinuation) in
            searchPlaceIndexForSuggestions(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
