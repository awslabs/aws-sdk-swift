...
extension LocationClient: LocationClientProtocol {
    ...
    public func searchPlaceIndexForSuggestions(input: SearchPlaceIndexForSuggestionsInput, completion: @escaping (ClientRuntime.SdkResult<SearchPlaceIndexForSuggestionsOutputResponse, SearchPlaceIndexForSuggestionsOutputError>) -> Void)
    {
        ...
        operation.initializeStep.intercept(position: .after, middleware: SearchPlaceIndexForSuggestionsInputURLPathMiddleware())
        operation.initializeStep.intercept(position: .after, middleware: SearchPlaceIndexForSuggestionsInputURLHostMiddleware())
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.EndpointResolverMiddleware(endpointResolver: config.endpointResolver, serviceId: serviceName))
        let apiMetadata = AWSClientRuntime.APIMetadata(serviceId: serviceName, version: "1.0")
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.UserAgentMiddleware(metadata: AWSClientRuntime.AWSUserAgentMetadata.fromEnv(apiMetadata: apiMetadata, frameworkMetadata: config.frameworkMetadata)))
        operation.serializeStep.intercept(position: .after, middleware: SearchPlaceIndexForSuggestionsInputHeadersMiddleware())
        operation.serializeStep.intercept(position: .after, middleware: SearchPlaceIndexForSuggestionsInputQueryItemMiddleware())
        ...
        operation.serializeStep.intercept(position: .after, middleware: SearchPlaceIndexForSuggestionsInputBodyMiddleware())
        operation.finalizeStep.intercept(position: .before, middleware: ClientRuntime.ContentLengthMiddleware())
        operation.finalizeStep.intercept(position: .after, middleware: AWSClientRuntime.RetryerMiddleware(retryer: config.retryer))
        let sigv4Config = AWSClientRuntime.SigV4Config(unsignedBody: false)
        operation.finalizeStep.intercept(position: .before, middleware: AWSClientRuntime.SigV4Middleware(config: sigv4Config))
