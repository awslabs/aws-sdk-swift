...
extension DataExchangeClient: DataExchangeClientProtocol {
    ...
    public func sendApiAsset(input: SendApiAssetInput, completion: @escaping (ClientRuntime.SdkResult<SendApiAssetOutputResponse, SendApiAssetOutputError>) -> Void)
    {
        ...
        operation.initializeStep.intercept(position: .after, middleware: SendApiAssetInputURLPathMiddleware())
        operation.initializeStep.intercept(position: .after, middleware: SendApiAssetInputURLHostMiddleware())
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.EndpointResolverMiddleware(endpointResolver: config.endpointResolver, serviceId: serviceName))
        let apiMetadata = AWSClientRuntime.APIMetadata(serviceId: serviceName, version: "1.0")
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.UserAgentMiddleware(metadata: AWSClientRuntime.AWSUserAgentMetadata.fromEnv(apiMetadata: apiMetadata, frameworkMetadata: config.frameworkMetadata)))
        operation.serializeStep.intercept(position: .after, middleware: SendApiAssetInputHeadersMiddleware())
        operation.serializeStep.intercept(position: .after, middleware: SendApiAssetInputQueryItemMiddleware())
