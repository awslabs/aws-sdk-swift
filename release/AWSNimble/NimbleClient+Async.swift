...
public extension NimbleClient {
    ...
    {
        typealias startStreamingSessionContinuation = CheckedContinuation<StartStreamingSessionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: startStreamingSessionContinuation) in
            startStreamingSession(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
    ...
    {
        typealias stopStreamingSessionContinuation = CheckedContinuation<StopStreamingSessionOutputResponse, Swift.Error>
        return try await withCheckedThrowingContinuation { (continuation: stopStreamingSessionContinuation) in
            stopStreamingSession(input: input) { result in
                switch result {
                    case .success(let output):
                        continuation.resume(returning: output)
                    case .failure(let error):
