...
extension ApiGatewayManagementApiClient: ApiGatewayManagementApiClientProtocol {
    ...
    public func getConnection(input: GetConnectionInput, completion: @escaping (SdkResult<GetConnectionOutput, GetConnectionOutputError>) -> Void)
    {
        ...
        operation.serializeStep.intercept(position: .before, middleware: ContentTypeMiddleware<GetConnectionInput, GetConnectionOutput, GetConnectionOutputError>(contentType: "application/json"))
        operation.deserializeStep.intercept(position: .before, middleware: LoggerMiddleware(clientLogMode: config.clientLogMode))
        operation.buildStep.intercept(position: .before, middleware: EndpointResolverMiddleware(endpointResolver: config.endpointResolver, serviceId: serviceName))
        operation.finalizeStep.intercept(position: .after, middleware: RetrierMiddleware(retrier: config.retrier))
        let sigv4Config = SigV4Config(unsignedBody: false)
    ...
    public func postToConnection(input: PostToConnectionInput, completion: @escaping (SdkResult<PostToConnectionOutput, PostToConnectionOutputError>) -> Void)
    {
        guard let connectionId = input.connectionId else {
            completion(.failure(.client(ClientError.serializationFailed("uri component connectionId unexpectedly nil"))))
            return
        ...
        let context = HttpContextBuilder()
                      ...
                      .withSigningName(value: "execute-api")
                      .withSigningRegion(value: config.signingRegion)
        var operation = OperationStack<PostToConnectionInput, PostToConnectionOutput, PostToConnectionOutputError>(id: "postToConnection")
        operation.addDefaultOperationMiddlewares()
        ...
        operation.serializeStep.intercept(position: .before, middleware: ContentTypeMiddleware<PostToConnectionInput, PostToConnectionOutput, PostToConnectionOutputError>(contentType: "application/json"))
