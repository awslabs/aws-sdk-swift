...
public extension Process {
    ...
    convenience init(_ argsList: [String]) {
        self.init(envVars: [:], argsList: argsList)
    }

    /// Creates a process using `/usr/bin/env` as the executable, and sets `envVars`before running.
    ///
    /// This makes it easy to create a process  for any command, as long as the corresponding executable exists in the PATH.
    ///
    /// - Parameter argsList: The command arguments, such as `["swift", "build"]`
    convenience init(envVars: [String: String], argsList: [String]) {
        self.init()
        self.environment = (self.environment ?? [:]).merging(envVars) { _, new in new }
