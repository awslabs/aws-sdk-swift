...
let package = Package(
    ...
    targets: [
        ...
        .target(
            name: "AWSSDKHTTPAuth",
            dependencies: [.crt, .smithy, .clientRuntime, .smithyChecksumsAPI, .smithyHTTPAuth, "AWSSDKIdentity"],
...
func addIntegrationTestTarget(_ name: String) {
    ...
    package.targets += [
        .testTarget(
            ...
            dependencies: [.crt, .clientRuntime, .awsClientRuntime, .byName(name: name), .smithyTestUtils, .awsSDKIdentity] + additionalDependencies.map { Target.Dependency.target(name: $0, condition: nil) },
