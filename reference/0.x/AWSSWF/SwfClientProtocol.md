...
If the caller doesn't have sufficient permissions to invoke the action, or the parameter values fall outside the specified constraints, the action fails. The associated event attribute's cause parameter is set to OPERATION\_NOT\_PERMITTED. For details and example IAM policies, see [Using IAM to Manage Access to Amazon SWF Workflows](https://docs.aws.amazon.com/amazonswf/latest/developerguide/swf-dev-iam.html) in the Amazon SWF Developer Guide.
...
### countPendingDecisionTasks(input:completion:)
...
### describeActivityType(input:completion:)
...
### getWorkflowExecutionHistory(input:completion:)
...
### listOpenWorkflowExecutions(input:completion:)
...
### respondActivityTaskCompleted(input:completion:)
...
### respondActivityTaskFailed(input:completion:)
