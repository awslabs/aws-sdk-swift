...
class FieldConfigurerTests: XCTestCase {
    ...
    var fileBasedConfig: FileBasedConfiguration = try! CRTFileBasedConfiguration(
        configFilePath: Bundle.module.path(forResource: "field_configuration_tests", ofType: nil)!,
        credentialsFilePath: nil
    )

    func test_value_itReturnsTheConfigValue() {
        let expected = Int.random(in: 1...Int.max)
        let subject = FieldConfigurer<Int>(configValue: expected, envVarName: envVarName, configFieldName: configFieldName, fileBasedConfig: fileBasedConfig, profileName: nil, converter: { Int($0) })
    ...
    func test_value_itReturnsTheEnvironmentValueWhenNoConfigValueIsSet() {
        ...
        let subject = FieldConfigurer<Int>(configValue: nil, envVarName: envVarName, configFieldName: configFieldName, fileBasedConfig: fileBasedConfig, profileName: nil, converter: { Int($0) })
        XCTAssertEqual(subject.value, expected)
        unsetenv(envVarName)
    }
    ...
    func test_value_itReturnsTheConfigFileValueFromAltProfile1WhenProfileSpecifiedInParam() {
        let expected = 456
        let subject = FieldConfigurer<Int>(configValue: nil, envVarName: envVarName, configFieldName: configFieldName, fileBasedConfig: fileBasedConfig, profileName: "alt-profile-1", converter: { Int($0) })
        XCTAssertEqual(subject.value, expected)
    }
    ...
    func test_value_itReturnsNilWhenNoSourceIsSet() {
        let subject = FieldConfigurer<Int>(configValue: nil, envVarName: envVarName, configFieldName: configFieldName, fileBasedConfig: fileBasedConfig, profileName: "no-such-profile", converter: { Int($0) })
