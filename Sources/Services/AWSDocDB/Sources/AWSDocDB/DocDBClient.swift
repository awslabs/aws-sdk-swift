...
extension DocDBClient {
    ...
    public func switchoverGlobalCluster(input: SwitchoverGlobalClusterInput) async throws -> SwitchoverGlobalClusterOutput {
        let context = Smithy.ContextBuilder()
                      ...
                      .withSigningRegion(value: config.signingRegion)
                      .build()
        let builder = ClientRuntime.OrchestratorBuilder<SwitchoverGlobalClusterInput, SwitchoverGlobalClusterOutput, SmithyHTTPAPI.SdkHttpRequest, SmithyHTTPAPI.HttpResponse>()
        config.interceptorProviders.forEach { provider in
            builder.interceptors.add(provider.create())
        }
        config.httpInterceptorProviders.forEach { provider in
            let i: any ClientRuntime.HttpInterceptor<SwitchoverGlobalClusterInput, SwitchoverGlobalClusterOutput> = provider.create()
            builder.interceptors.add(i)
        }
        ...
        builder.interceptors.add(AWSClientRuntime.UserAgentMiddleware<SwitchoverGlobalClusterInput, SwitchoverGlobalClusterOutput>(metadata: AWSClientRuntime.AWSUserAgentMetadata.fromConfig(serviceID: serviceName, version: "1.0", config: config)))
