...
public struct SagemakerEdgeAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: String
    // Region is used for SigV4 auth scheme
    public let region: String?
}

public protocol SagemakerEdgeAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service SagemakerEdge must conform to.
}

public struct DefaultSagemakerEdgeAuthSchemeResolver: SagemakerEdgeAuthSchemeResolver {
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [AuthOption] {
        ...
        guard let serviceParams = params as? SagemakerEdgeAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return SagemakerEdgeAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
