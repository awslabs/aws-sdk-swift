...
public class NovaActClientEndpointPlugin: Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        guard var config = clientConfiguration as? NovaActClient.NovaActClientConfig else { return }
...
public class DefaultAWSAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        if var config = clientConfiguration as? NovaActClient.NovaActClientConfig {
            config.authSchemeResolver = DefaultNovaActAuthSchemeResolver()
...
public class NovaActClientAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public init(authSchemes: SmithyHTTPAuthAPI.AuthSchemes? = nil, authSchemePreference: [String]? = nil, authSchemeResolver: NovaActAuthSchemeResolver? = nil, awsCredentialIdentityResolver: (any SmithyIdentity.AWSCredentialIdentityResolver)? = nil, bearerTokenIdentityResolver: (any SmithyIdentity.BearerTokenIdentityResolver)? = nil) {
