...
public struct EchoCBORAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: Swift.String
    // Region is used for SigV4 auth scheme
    public let region: Swift.String?
}

public protocol EchoCBORAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service EchoCBOR must conform to.
}

public struct DefaultEchoCBORAuthSchemeResolver: EchoCBORAuthSchemeResolver {
    ...
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [SmithyHTTPAuthAPI.AuthOption] {
        ...
        guard let serviceParams = params as? EchoCBORAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return EchoCBORAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
