...
public struct SecretsManagerCBORAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: Swift.String
    // Region is used for SigV4 auth scheme
    public let region: Swift.String?
}

public protocol SecretsManagerCBORAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service SecretsManagerCBOR must conform to.
}

public struct DefaultSecretsManagerCBORAuthSchemeResolver: SecretsManagerCBORAuthSchemeResolver {
    ...
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [SmithyHTTPAuthAPI.AuthOption] {
        ...
        guard let serviceParams = params as? SecretsManagerCBORAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return SecretsManagerCBORAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
