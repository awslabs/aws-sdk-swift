...
extension CreateChatTokenOutput {
    ...
    static func httpOutput(from httpResponse: SmithyHTTPAPI.HttpResponse) async throws -> CreateChatTokenOutput {
        ...
        value.sessionExpirationTime = try reader["sessionExpirationTime"].readTimestampIfPresent(format: .dateTime)
        value.token = try reader["token"].readIfPresent()
        value.tokenExpirationTime = try reader["tokenExpirationTime"].readTimestampIfPresent(format: .dateTime)
        return value
    }
}
