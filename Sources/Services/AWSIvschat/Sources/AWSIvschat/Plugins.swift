...
public class IvschatClientEndpointPlugin: Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        guard var config = clientConfiguration as? IvschatClient.IvschatClientConfig else { return }
...
public class DefaultAWSAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        if var config = clientConfiguration as? IvschatClient.IvschatClientConfig {
            config.authSchemeResolver = DefaultIvschatAuthSchemeResolver()
...
public class IvschatClientAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public init(authSchemes: SmithyHTTPAuthAPI.AuthSchemes? = nil, authSchemePreference: [String]? = nil, authSchemeResolver: IvschatAuthSchemeResolver? = nil, awsCredentialIdentityResolver: (any SmithyIdentity.AWSCredentialIdentityResolver)? = nil, bearerTokenIdentityResolver: (any SmithyIdentity.BearerTokenIdentityResolver)? = nil) {
