...
extension AccessAnalyzerClient {
    /// Paginate over `[ListPolicyPreviewJobsOutput]` results.
    ///
    /// When this operation is called, an `AsyncSequence` is created. AsyncSequences are lazy so no service
    /// calls are made until the sequence is iterated over. This also means there is no guarantee that the request is valid
    /// until then. If there are errors in your request, you will see the failures only after you start iterating.
    /// - Parameters:
    ///     - input: A `[ListPolicyPreviewJobsInput]` to start pagination
    /// - Returns: An `AsyncSequence` that can iterate over `ListPolicyPreviewJobsOutput`
    public func listPolicyPreviewJobsPaginated(input: ListPolicyPreviewJobsInput) -> ClientRuntime.PaginatorSequence<ListPolicyPreviewJobsInput, ListPolicyPreviewJobsOutput> {
        return ClientRuntime.PaginatorSequence<ListPolicyPreviewJobsInput, ListPolicyPreviewJobsOutput>(input: input, inputKey: \.nextToken, outputKey: \.nextToken, paginationFunction: self.listPolicyPreviewJobs(input:))
    }
}

extension ListPolicyPreviewJobsInput: ClientRuntime.PaginateToken {
    public func usingPaginationToken(_ token: Swift.String) -> ListPolicyPreviewJobsInput {
        return ListPolicyPreviewJobsInput(
...
extension PaginatorSequence where OperationStackInput == ListPolicyPreviewJobsInput, OperationStackOutput == ListPolicyPreviewJobsOutput {
    /// This paginator transforms the `AsyncSequence` returned by `listPolicyPreviewJobsPaginated`
    /// to access the nested member `[AccessAnalyzerClientTypes.PolicyPreviewAnalysisReport]`
    /// - Returns: `[AccessAnalyzerClientTypes.PolicyPreviewAnalysisReport]`
    public func analysisReports() async throws -> [AccessAnalyzerClientTypes.PolicyPreviewAnalysisReport] {
