...
extension CloudFormationClient {
    /// Paginate over `[DescribeChangeSetOutput]` results.
    ///
    /// When this operation is called, an `AsyncSequence` is created. AsyncSequences are lazy so no service
    /// calls are made until the sequence is iterated over. This also means there is no guarantee that the request is valid
    /// until then. If there are errors in your request, you will see the failures only after you start iterating.
    /// - Parameters:
    ///     - input: A `[DescribeChangeSetInput]` to start pagination
    /// - Returns: An `AsyncSequence` that can iterate over `DescribeChangeSetOutput`
    public func describeChangeSetPaginated(input: DescribeChangeSetInput) -> ClientRuntime.PaginatorSequence<DescribeChangeSetInput, DescribeChangeSetOutput> {
        return ClientRuntime.PaginatorSequence<DescribeChangeSetInput, DescribeChangeSetOutput>(input: input, inputKey: \.nextToken, outputKey: \.nextToken, paginationFunction: self.describeChangeSet(input:))
    }
}

extension DescribeChangeSetInput: ClientRuntime.PaginateToken {
    public func usingPaginationToken(_ token: Swift.String) -> DescribeChangeSetInput {
        return DescribeChangeSetInput(
...
extension PaginatorSequence where OperationStackInput == DescribeChangeSetInput, OperationStackOutput == DescribeChangeSetOutput {
    /// This paginator transforms the `AsyncSequence` returned by `describeChangeSetPaginated`
    /// to access the nested member `[CloudFormationClientTypes.Change]`
    /// - Returns: `[CloudFormationClientTypes.Change]`
    public func changes() async throws -> [CloudFormationClientTypes.Change] {
        return try await self.asyncCompactMap { item in item.changes }
    }
}
extension CloudFormationClient {
...
extension PaginatorSequence where OperationStackInput == DescribeEventsInput, OperationStackOutput == DescribeEventsOutput {
    ...
    /// to access the nested member `[CloudFormationClientTypes.OperationEvent]`
    /// - Returns: `[CloudFormationClientTypes.OperationEvent]`
    public func operationEvents() async throws -> [CloudFormationClientTypes.OperationEvent] {
