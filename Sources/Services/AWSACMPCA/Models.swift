...
extension CreatePermissionInput {
    ...
    static func write(value: CreatePermissionInput?, to writer: SmithyJSON.Writer) throws {
        guard let value else { return }
        try writer["Actions"].writeList(value.actions, memberWritingClosure: ACMPCAClientTypes.ActionType.write(value:to:), memberNodeInfo: "member", isFlattened: false)
        try writer["CertificateAuthorityArn"].write(value.certificateAuthorityArn)
...
extension ACMPCAClientTypes.CertificateAuthority {
    ...
    static func read(from reader: SmithyJSON.Reader) throws -> ACMPCAClientTypes.CertificateAuthority {
        ...
        value.createdAt = try reader["CreatedAt"].readTimestampIfPresent(format: .epochSeconds)
        value.lastStateChangeAt = try reader["LastStateChangeAt"].readTimestampIfPresent(format: .epochSeconds)
        value.type = try reader["Type"].readIfPresent()
        value.serial = try reader["Serial"].readIfPresent()
        value.status = try reader["Status"].readIfPresent()
        value.notBefore = try reader["NotBefore"].readTimestampIfPresent(format: .epochSeconds)
        value.notAfter = try reader["NotAfter"].readTimestampIfPresent(format: .epochSeconds)
        ...
        value.restorableUntil = try reader["RestorableUntil"].readTimestampIfPresent(format: .epochSeconds)
...
extension ACMPCAClientTypes.Permission {
    ...
    static func read(from reader: SmithyJSON.Reader) throws -> ACMPCAClientTypes.Permission {
        ...
        value.actions = try reader["Actions"].readListIfPresent(memberReadingClosure: ACMPCAClientTypes.ActionType.read(from:), memberNodeInfo: "member", isFlattened: false)
        value.policy = try reader["Policy"].readIfPresent()
        return value
    }
}
