...
public class MigrationHubRefactorSpacesClient {
    ...
    public class MigrationHubRefactorSpacesClientConfiguration: MigrationHubRefactorSpacesClientConfigurationProtocol {
        ...
        public init(
            ...
            let runtimeConfig = try runtimeConfig ?? ClientRuntime.DefaultSDKRuntimeConfiguration("MigrationHubRefactorSpacesClient")
...
extension MigrationHubRefactorSpacesClient: MigrationHubRefactorSpacesClientProtocol {
    ...
    public func getRoute(input: GetRouteInput) async throws -> GetRouteOutputResponse
    {
        ...
        guard let region = config.region else {
            throw SdkError<GetRouteOutputError>.client(ClientError.unknownError(("Missing required parameter: Region")))
        }
        let endpointParams = EndpointParams(endpoint: config.endpoint, region: region, useDualStack: config.useDualStack ?? false, useFIPS: config.useFIPS ?? false)
    ...
    public func listEnvironmentVpcs(input: ListEnvironmentVpcsInput) async throws -> ListEnvironmentVpcsOutputResponse
    {
        ...
        guard let region = config.region else {
            throw SdkError<ListEnvironmentVpcsOutputError>.client(ClientError.unknownError(("Missing required parameter: Region")))
        }
        let endpointParams = EndpointParams(endpoint: config.endpoint, region: region, useDualStack: config.useDualStack ?? false, useFIPS: config.useFIPS ?? false)
