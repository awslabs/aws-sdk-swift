...
extension ListFHIRExportJobsInput {
    ...
    static func write(value: ListFHIRExportJobsInput?, to writer: SmithyJSON.Writer) throws {
        ...
        try writer["MaxResults"].write(value.maxResults)
        try writer["NextToken"].write(value.nextToken)
        try writer["SubmittedAfter"].writeTimestamp(value.submittedAfter, format: .epochSeconds)
        try writer["SubmittedBefore"].writeTimestamp(value.submittedBefore, format: .epochSeconds)
    }
}
