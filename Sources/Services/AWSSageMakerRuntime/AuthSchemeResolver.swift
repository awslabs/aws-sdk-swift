...
public struct SageMakerRuntimeAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: String
    // Region is used for SigV4 auth scheme
    public let region: String?
}

public protocol SageMakerRuntimeAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service SageMakerRuntime must conform to.
}

public struct DefaultSageMakerRuntimeAuthSchemeResolver: SageMakerRuntimeAuthSchemeResolver {
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [AuthOption] {
        ...
        guard let serviceParams = params as? SageMakerRuntimeAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return SageMakerRuntimeAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
