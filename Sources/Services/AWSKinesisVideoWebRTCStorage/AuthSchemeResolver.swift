...
public struct KinesisVideoWebRTCStorageAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: String
    // Region is used for SigV4 auth scheme
    public let region: String?
}

public protocol KinesisVideoWebRTCStorageAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service KinesisVideoWebRTCStorage must conform to.
}

public struct DefaultKinesisVideoWebRTCStorageAuthSchemeResolver: KinesisVideoWebRTCStorageAuthSchemeResolver {
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [AuthOption] {
        ...
        guard let serviceParams = params as? KinesisVideoWebRTCStorageAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return KinesisVideoWebRTCStorageAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
