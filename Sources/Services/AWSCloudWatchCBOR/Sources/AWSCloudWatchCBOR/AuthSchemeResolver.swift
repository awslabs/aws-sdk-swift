...
public struct CloudWatchCBORAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: Swift.String
    // Region is used for SigV4 auth scheme
    public let region: Swift.String?
}

public protocol CloudWatchCBORAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service CloudWatchCBOR must conform to.
}

public struct DefaultCloudWatchCBORAuthSchemeResolver: CloudWatchCBORAuthSchemeResolver {
    ...
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [SmithyHTTPAuthAPI.AuthOption] {
        ...
        guard let serviceParams = params as? CloudWatchCBORAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return CloudWatchCBORAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
