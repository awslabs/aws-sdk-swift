...
extension UpdateExpirationForHITInput {
    ...
    static func write(value: UpdateExpirationForHITInput?, to writer: SmithyJSON.Writer) throws {
        guard let value else { return }
        try writer["ExpireAt"].writeTimestamp(value.expireAt, format: .epochSeconds)
        try writer["HITId"].write(value.hitId)
    }
}
...
extension MTurkClientTypes.Assignment {
    ...
    static func read(from reader: SmithyJSON.Reader) throws -> MTurkClientTypes.Assignment {
        ...
        value.autoApprovalTime = try reader["AutoApprovalTime"].readTimestampIfPresent(format: .epochSeconds)
        value.acceptTime = try reader["AcceptTime"].readTimestampIfPresent(format: .epochSeconds)
        value.submitTime = try reader["SubmitTime"].readTimestampIfPresent(format: .epochSeconds)
        value.approvalTime = try reader["ApprovalTime"].readTimestampIfPresent(format: .epochSeconds)
        value.rejectionTime = try reader["RejectionTime"].readTimestampIfPresent(format: .epochSeconds)
        value.deadline = try reader["Deadline"].readTimestampIfPresent(format: .epochSeconds)
...
extension MTurkClientTypes.Qualification {
    ...
    static func read(from reader: SmithyJSON.Reader) throws -> MTurkClientTypes.Qualification {
        ...
        value.qualificationTypeId = try reader["QualificationTypeId"].readIfPresent()
        value.workerId = try reader["WorkerId"].readIfPresent()
        value.grantTime = try reader["GrantTime"].readTimestampIfPresent(format: .epochSeconds)
