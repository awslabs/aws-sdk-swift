...
public struct KinesisVideoMediaAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: String
    // Region is used for SigV4 auth scheme
    public let region: String?
}

public protocol KinesisVideoMediaAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service KinesisVideoMedia must conform to.
}

public struct DefaultKinesisVideoMediaAuthSchemeResolver: KinesisVideoMediaAuthSchemeResolver {
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [AuthOption] {
        ...
        guard let serviceParams = params as? KinesisVideoMediaAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return KinesisVideoMediaAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
