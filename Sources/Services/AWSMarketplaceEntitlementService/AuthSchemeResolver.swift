...
public struct MarketplaceEntitlementServiceAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: String
    // Region is used for SigV4 auth scheme
    public let region: String?
}

public protocol MarketplaceEntitlementServiceAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service MarketplaceEntitlementService must conform to.
}

public struct DefaultMarketplaceEntitlementServiceAuthSchemeResolver: MarketplaceEntitlementServiceAuthSchemeResolver {
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [AuthOption] {
        ...
        guard let serviceParams = params as? MarketplaceEntitlementServiceAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return MarketplaceEntitlementServiceAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
