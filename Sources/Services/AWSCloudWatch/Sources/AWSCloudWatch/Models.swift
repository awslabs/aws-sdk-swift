...
import func SmithyReadWrite.timestampReadingClosure
...
extension CloudWatchClientTypes.MetricDataResult {
    ...
    static func read(from reader: SmithyXML.Reader) throws -> CloudWatchClientTypes.MetricDataResult {
        ...
        value.id = try reader["Id"].readIfPresent()
        value.label = try reader["Label"].readIfPresent()
        value.timestamps = try reader["Timestamps"].readListIfPresent(memberReadingClosure: SmithyReadWrite.timestampReadingClosure(format: .dateTime), memberNodeInfo: "member", isFlattened: false)
