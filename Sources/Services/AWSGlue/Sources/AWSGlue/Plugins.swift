...
public class GlueClientEndpointPlugin: Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        guard var config = clientConfiguration as? GlueClient.GlueClientConfig else { return }
...
public class DefaultAWSAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        if var config = clientConfiguration as? GlueClient.GlueClientConfig {
            config.authSchemeResolver = DefaultGlueAuthSchemeResolver()
...
public class GlueClientAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public init(authSchemes: SmithyHTTPAuthAPI.AuthSchemes? = nil, authSchemePreference: [String]? = nil, authSchemeResolver: GlueAuthSchemeResolver? = nil, awsCredentialIdentityResolver: (any SmithyIdentity.AWSCredentialIdentityResolver)? = nil, bearerTokenIdentityResolver: (any SmithyIdentity.BearerTokenIdentityResolver)? = nil) {
