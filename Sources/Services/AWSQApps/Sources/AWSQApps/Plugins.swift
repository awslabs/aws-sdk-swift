...
public class QAppsClientEndpointPlugin: Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        guard var config = clientConfiguration as? QAppsClient.QAppsClientConfig else { return }
...
public class DefaultAWSAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        if var config = clientConfiguration as? QAppsClient.QAppsClientConfig {
            config.authSchemeResolver = DefaultQAppsAuthSchemeResolver()
...
public class QAppsClientAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public init(authSchemes: SmithyHTTPAuthAPI.AuthSchemes? = nil, authSchemePreference: [String]? = nil, authSchemeResolver: QAppsAuthSchemeResolver? = nil, awsCredentialIdentityResolver: (any SmithyIdentity.AWSCredentialIdentityResolver)? = nil, bearerTokenIdentityResolver: (any SmithyIdentity.BearerTokenIdentityResolver)? = nil) {
