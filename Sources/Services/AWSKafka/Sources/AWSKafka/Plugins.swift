...
public class KafkaClientEndpointPlugin: Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        guard var config = clientConfiguration as? KafkaClient.KafkaClientConfig else { return }
...
public class DefaultAWSAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public func configureClient<Config: ClientRuntime.ClientConfiguration>(clientConfiguration: inout Config) async throws {
        if var config = clientConfiguration as? KafkaClient.KafkaClientConfig {
            config.authSchemeResolver = DefaultKafkaAuthSchemeResolver()
...
public class KafkaClientAuthSchemePlugin: ClientRuntime.Plugin {
    ...
    public init(authSchemes: SmithyHTTPAuthAPI.AuthSchemes? = nil, authSchemePreference: [String]? = nil, authSchemeResolver: KafkaAuthSchemeResolver? = nil, awsCredentialIdentityResolver: (any SmithyIdentity.AWSCredentialIdentityResolver)? = nil, bearerTokenIdentityResolver: (any SmithyIdentity.BearerTokenIdentityResolver)? = nil) {
