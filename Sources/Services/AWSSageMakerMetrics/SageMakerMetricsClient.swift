...
extension SageMakerMetricsClient {
    ...
    public func batchPutMetrics(input: BatchPutMetricsInput) async throws -> BatchPutMetricsOutput {
        ...
        operation.buildStep.intercept(position: .before, middleware: AWSClientRuntime.UserAgentMiddleware<BatchPutMetricsOutput>(metadata: AWSClientRuntime.AWSUserAgentMetadata.fromConfig(serviceID: serviceName, version: "1.0", config: config)))
        ...
        operation.finalizeStep.intercept(position: .before, middleware: ClientRuntime.ContentLengthMiddleware<BatchPutMetricsOutput>())
