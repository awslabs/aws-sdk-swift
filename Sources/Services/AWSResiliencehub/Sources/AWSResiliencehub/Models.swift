...
extension DescribeDraftAppVersionResourcesImportStatusOutput {
    ...
    static func httpOutput(from httpResponse: SmithyHTTPAPI.HTTPResponse) async throws -> DescribeDraftAppVersionResourcesImportStatusOutput {
        ...
        value.errorMessage = try reader["errorMessage"].readIfPresent()
        value.status = try reader["status"].readIfPresent()
        value.statusChangeTime = try reader["statusChangeTime"].readTimestampIfPresent(format: .epochSeconds)
        return value
    }
}
...
extension ResiliencehubClientTypes.App {
    ...
    static func read(from reader: SmithyJSON.Reader) throws -> ResiliencehubClientTypes.App {
        ...
        value.lastAppComplianceEvaluationTime = try reader["lastAppComplianceEvaluationTime"].readTimestampIfPresent(format: .epochSeconds)
        value.resiliencyScore = try reader["resiliencyScore"].readIfPresent() ?? 0
        value.lastResiliencyScoreEvaluationTime = try reader["lastResiliencyScoreEvaluationTime"].readTimestampIfPresent(format: .epochSeconds)
        value.tags = try reader["tags"].readMapIfPresent(valueReadingClosure: SmithyReadWrite.ReadingClosures.readString(from:), keyNodeInfo: "key", valueNodeInfo: "value", isFlattened: false)
        ...
        value.lastDriftEvaluationTime = try reader["lastDriftEvaluationTime"].readTimestampIfPresent(format: .epochSeconds)
