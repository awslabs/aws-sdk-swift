...
public extension S3TransferManager {
    ...
    func downloadObject(input: DownloadObjectInput) throws -> Task<DownloadObjectOutput, Error> {
        return Task {
            ...
            if let range, downloadType == .part {
                return try await DownloadObjectOutput(
                    getObjectOutput: performSingleGET(s3, input.getObjectInput, input.outputStream)
                )
            }
    ...
    private func performMultipartGET(s3: S3Client, input: DownloadObjectInput) async throws -> DownloadObjectOutput {
    ...
    private func concurrentlyFetchPartGETByteStreams(s3: S3Client, input: DownloadObjectInput, partsCount: Int) async throws -> [ByteStream] {
    ...
    private func concurrentlyFetchRangeGETByteStreams(
        s3: S3Client,
        input: DownloadObjectInput,
        startByte: Int,
        endByte: Int? = nil, // Known only for "bytes=<start>-<end>".
    ...
    private func writeByteStreams(_ byteStreams: [ByteStream], to outputStream: OutputStream) async throws {
    ...
    private func writeData(_ data: Data, to outputStream: OutputStream) throws {
