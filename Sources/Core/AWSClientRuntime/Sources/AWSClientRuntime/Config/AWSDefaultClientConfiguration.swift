...
public protocol AWSDefaultClientConfiguration {
    ...
    /// Specifies whether the endpoint configured via environment variable or shared config file should be used by the service client.
    ///
    ...
    ///     2. The value of `AWS_ENDPOINT_URL_<SERVICE>` environment variable where `<SERVICE>` is the transformed value of the given service client's `serviceName` property, where spaces are replaced with underscores and letters are uppercased. E.g., `"API Gateway"` => `"API_GATEWAY"`.
    ///     3. The value of `AWS_ENDPOINT_URL` environment variable; this is used to configure the endpoint for all services.
    ///     4. In the shared config file, the `endpoint_url` property under selected profile's `services` section's `<SERVICE> =` subsection. The `<SERVICE>` value is the transformed value of the given service client's `serviceName` property, where spaces are replaced with underscores and letters are lowercased. E.g., `"API Gateway` => `"api_gateway`.
    ...
    ///     1. The `endpoint` value provided at service client initialization via the config.
    ///     2. If `endpoint` wasn't configured by the user via service client config, AWS Swift SDK determines the endpoint to use for each operation call by using the built-in endpoint reoslver for the service.
    ///
    /// If this flag isn't set, AWS Swift SDK look at the corresponding environment variable and shared config file property when `endpoint` service client config is resolved. The flag value gets dermined by looking at the possible sources in the following order:
    ...
    ///     4. If this flag was not set anywhere, the AWS Swift SDK attempts to resolve configured endpoint.
    ...
    ///
    /// For more information on service-specific endpoints, see the [official AWS documentation](https://docs.aws.amazon.com/sdkref/latest/guide/feature-ss-endpoints.html).
    ///
