...
public class AWSSigV4Signer: ClientRuntime.Signer {
    ...
    private func constructSigningConfig(
        ...
        let signedBodyValue: AWSSignedBodyValue = determineSignedBodyValue(with: checksum, is: unsignedBody)
    ...
    private func determineSignedBodyValue(
        ...
        using isUnsignedBody: Bool
    ) throws -> AWSSignedBodyValue {
        if checksum == nil {
            return isUnsignedBody ? .unsignedBody : empty
        } else {
            // checksum is present
