...
class AWSSmokeTestGenerator(
    ...
    override fun renderCustomFilePrivateVariables(writer: SwiftWriter) {
        ...
        writer.write(
            "fileprivate let tagsToSkip = (ProcessInfo.processInfo.environment[\"AWS_SMOKE_TEST_SKIP_TAGS\"] ?? \"\").components(separatedBy: \",\")",
        )
    }

    override fun handleVendorParams(
    ...
    private fun getFormattedVendorParams(vendorParams: ObjectNode): Map<String, String> {
        ...
        vendorParams.members.forEach { originalMapping ->
            when (originalMapping.key.value) {
                ...
                "uri" -> {
                    formattedMapping.put("endpoint", "\"${originalMapping.value.expectStringNode().value}\"")
                }
                "useFips" -> {
                    formattedMapping.put(
                        "useFIPS",
