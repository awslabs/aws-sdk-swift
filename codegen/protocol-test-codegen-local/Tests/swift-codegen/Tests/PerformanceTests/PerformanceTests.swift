...
final class PerformanceTests: XCTestCase {
    ...
    // MARK: - ReadWrite JSON performance tests
    ...
    // MARK: - Swift Decodable JSON performance tests
    ...
    private static func generateData(n: Int) throws -> ([TestStructure], Data) {
        ...
        // Create an array of TestStructures with n elements, filled with randomized data
        // (int is the index of the TestStructure in the array.)
        let array = (0..<n).map { i in
            TestStructure(
                bool: Bool.random(),
                int: i,
                listOfInts: (0..<100).map { _ in Int.random(in: 0...1000) },
                mapOfDoubles: Dictionary(uniqueKeysWithValues: (
                    (0..<100).map { _ in (UUID().uuidString, Double.random(in: 0...1)) }
                )),
                string: UUID().uuidString
            )
        }

        // Write the TestStructure array to JSON-encoded data.  The SmithyJSON writer is used but Codable may be used as well:
        // (Tests have been run using either encoding mechanism, with the same results)
        //
...
// Here is TestStructure, as generated by protocol test tooling:
