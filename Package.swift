...
private let serviceClientData: [ServiceClientData] = [
    ...
    .init(
        ...
        [.AWSClientRuntime, .AWSSDKChecksums, .AWSSDKHTTPAuth, .AWSSDKIdentity, .ClientRuntime, .Smithy, .SmithyCBOR, .SmithyHTTPAPI, .SmithyHTTPAuthAPI, .SmithyIdentity, .SmithyReadWrite, .SmithyRetries, .SmithyRetriesAPI, .SmithyTimestamps]
    ),
    .init(
        "AWSComputeOptimizer",
...
private var selectedServiceClientData: [ServiceClientData] {
    ...
    guard let batchNumberString = env["AWS_SWIFT_SDK_BATCH_NUMBER"],
          let batchTotalString = env["AWS_SWIFT_SDK_BATCH_TOTAL"] else {
        return serviceClientData
    }
    guard let batchNumber = UInt(batchNumberString), let batchTotal = UInt(batchTotalString) else {
        return serviceClientData
    }

    // Don't do more batches than there are services
    let numberOfBatches = min(batchTotal, UInt(serviceClientData.count))

    // If there are more batches than services, don't include any services
    // in any excess batch
    guard batchNumber < numberOfBatches else { return [] }
    return serviceClientData.enumerated().filter {
        UInt($0.offset) % numberOfBatches == batchNumber
    }.map { $0.element }
}
